...
contract PriceFetcher {
    ...
    function getAmtSg(string memory tokenASymbol, uint amountADesired, string memory tokenBSymbol, uint amountBDesired) external view returns (int) {
        ...
        (, priceA, , ,) = AggregatorV3Interface(assetAadr).latestRoundData();

        // Check if amountBDesired is zero
        if (amountBDesired == 0) {
            // Set priceB to zero and assetBadr to address(0)
            priceB = 0;
            assetBadr = address(0);
        } else {
            // Get assetBadr and priceB if amountBDesired is not zero
            assetBadr = symbolToAddr[tokenBSymbol];
            (, priceB, , ,) = AggregatorV3Interface(assetBadr).latestRoundData();
        }
