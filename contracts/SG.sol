...
interface Ilogic {
    ...
    function getAmtSgBorrow(string memory tokenSymbol,uint amountDesired) external view returns (uint);
    function getAmtSgRepay(string memory tokenSymbol, uint amountDesired) external view returns (uint);
}
...
contract Sakha is ERC20 {
    ...
    function reward(address to, address tokenA, address tokenB, uint amountADesired, uint amountBDesired) external onlyRouter {
        ...
        // Check if tokenB is the "dead address"
        if (tokenB == address(0)) {
            // Set tokenBsymbol to 'none'
            tokenBsymbol = 'none';
        } else {
            // Call IEERC20(tokenB).symbol() if tokenB is a valid address
            tokenBsymbol = IEERC20(tokenB).symbol();
        }
    ...
    function rewardBorrow(address to,address token, uint amountDesired) external onlyRouter {

       string memory tokenSymbol = IEERC20(token).symbol();
 
        
        uint amtToken = Ilogic(logic).getAmtSgBorrow(tokenSymbol,amountDesired);
        _mint(to, amtToken* 10 ** 18);
    }

    function rewardRepay(address to,address token, uint amountDesired) external onlyRouter {

       string memory tokenSymbol = IEERC20(token).symbol();
        
        uint amtToken = Ilogic(logic).getAmtSgRepay(tokenSymbol,amountDesired);
        _mint(to, amtToken* 10 ** 18);
    }
